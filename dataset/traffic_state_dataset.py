...
from dataset.util import generate_dataloader
from utils.normalization import NormalScaler, StandardScaler, MinMax01Scaler, MinMax11Scaler, LogScaler, NoneScaler
...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        if not os.path.exists(self.data_path):
            raise ValueError(
                "Dataset {} not exist! Please ensure the path './data/{}/' exist!".format(self.dataset, self.dataset))
        # 加载数据集的config.json文件
    ...
    def _load_dtw(self):
        self.adj_dtw = np.zeros((len(self.geo_ids), len(self.geo_ids)), dtype=np.float32)
        train_data, _, _ = self._split_train_val_test()  # (len_time, num_nodes, feature_dim)
        for i in tqdm(range(len(self.geo_ids))):
            for j in range(len(self.geo_ids)):
                self.adj_dtw[i, j], _ = fastdtw(train_data[:, i, :], train_data[:, j, :], radius=6)
        distances = self.adj_dtw[~np.isinf(self.adj_dtw)].flatten()
        std = distances.std()
        self.adj_dtw = np.exp(-np.square(self.adj_dtw / std))
        self.adj_dtw[self.adj_dtw < self.weight_adj_epsilon] = 0
        self._logger.info("Generate grid rel file, shape=" + str(self.adj_dtw.shape))

    def _load_cache_adj(self):
        self._logger.info('Loading ' + os.path.join(self.cache_file_folder, "adj.npz"))
        cat_data = np.load(os.path.join(self.cache_file_folder, self.dataset + "_adj.npz"))
    ...
    def _split_train_val_test(self):
        ...
        if self.load_external and os.path.exists(self.data_path + self.ext_file + '.ext'):  # 外部数据集
            ...
            ext_data = None
        df = self._load_dyna(data_files)  # (len_time, ..., feature_dim)
        # if self.load_external:
        df = self._add_external_information(df, ext_data)
        seq_len = len(df)
        train_len = int(self.train_rate * seq_len)
        val_len = int(self.eval_rate * seq_len)
        ...
        self._logger.info("Dataset created")
        self._logger.info(
            "train shape: " + str(train_data.shape) + ", val shape: " + str(val_data.shape) + ", test shape: " + str(
                test_data.shape))
        return train_data, val_data, test_data

    def _split_x_y(self, train_data, test_data, val_data):
        x_train, y_train = self._generate_input_data(train_data)
        x_val, y_val = self._generate_input_data(val_data)
        x_test, y_test = self._generate_input_data(test_data)
        self._logger.info("train\t" + "x: " + str(x_train.shape) + ", y: " + str(y_train.shape))
        self._logger.info("eval\t" + "x: " + str(x_val.shape) + ", y: " + str(y_val.shape))
        self._logger.info("test\t" + "x: " + str(x_test.shape) + ", y: " + str(y_test.shape))
        if self.cache_dataset:
    ...
    def _generate_train_val_test(self):
        ...
        Returns:
            tuple: tuple contains:
                ...
                y_test: (num_samples, input_length, ..., feature_dim)
        """
        train_data, val_data, test_data = self._split_train_val_test()
        return self._split_x_y(train_data, test_data, val_data)
